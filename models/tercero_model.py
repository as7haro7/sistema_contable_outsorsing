...
class TerceroModel:
    ...
    def create(data, tipo):
        ...
        if tipo == 'cliente':
            query = """
                INSERT INTO cliente (razon, nit, telf, celular, email, pais, depto, domicilio, usuario, fechasys)
            ...
            query = """
                INSERT INTO proveedor (razon, nit, autorizacion, telf, celular, email, pais, depto, domicilio, usuario, fechasys)
    ...
    def update(tercero_id, data, tipo):
        ...
        if tipo == 'cliente':
            query = """
                UPDATE cliente
                SET razon=%s, nit=%s, telf=%s, celular=%s, email=%s, pais=%s, depto=%s, domicilio=%s, usuario=%s, fechasys=NOW()
                WHERE id=%s
            """
            params = (
                data['razon'], data['nit'], data.get('telf', ''), data.get('celular', ''),
                data.get('email', ''), data.get('pais', ''), data.get('depto', ''),
                data.get('domicilio', ''), usuario, tercero_id
            )
        elif tipo == 'proveedor':
            query = """
                UPDATE proveedor
                SET razon=%s, nit=%s, autorizacion=%s, telf=%s, celular=%s, email=%s, pais=%s, depto=%s, domicilio=%s, usuario=%s, fechasys=NOW()
                WHERE id=%s
            """
            params = (
                data['razon'], data['nit'], data.get('autorizacion', ''), data.get('telf', ''),
                data.get('celular', ''), data.get('email', ''), data.get('pais', ''),
                data.get('depto', ''), data.get('domicilio', ''), usuario, tercero_id
            )
        else:
            return None
